...
- For Rhino/Java primitives, use `flow-block-create` with `implementation.runtime: "rhino"` and a small `run(ctx,node)` implementation. Java classes are available through `Packages`; keep the Flow orchestration in FlowScript.
